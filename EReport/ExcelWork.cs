...
namespace EReport
{
    class ExcelWork
    {
        public int ExcelPlot_with_Compare(ref Excel.Worksheet _xlWorkSheet, ref DataSet ds1, int colNum, string name, string operator_type)
        {
            ...
            Excel.Range _range2 = (Excel.Range)_xlWorkSheet.Cells[1, colNum + 2];
            ...
            _xlWorkSheet.Cells[2, colNum + 2] = "Total Minutes";
            ...
            _xlWorkSheet.Cells[rowNum, colNum + 1] = "Sum";
            ...
            _xlWorkSheet.Cells[rowNum, colNum + 2].NumberFormat = "#,##0.00";
            ...
            Excel.Range _rangeB = (Excel.Range)_xlWorkSheet.Cells[rowNum, colNum + 2];
        ...
        public int ExcelPlot_without_Compare(ref Excel.Worksheet _xlWorkSheet, ref DataSet ds1, int rowNum, string name, string operator_type)
        {
            ...
            Excel.Range _range2 = (Excel.Range)_xlWorkSheet.Cells[rowNum, 3];
            Excel.Range workSheet_range = _xlWorkSheet.get_Range(_range1, _range2);
            workSheet_range.Merge();
            workSheet_range.HorizontalAlignment = Excel.XlHAlign.xlHAlignCenter;
            workSheet_range.Interior.Color = Excel.XlRgbColor.rgbLavender;
            ...
            _xlWorkSheet.Cells[rowNum, 3] = "Total Minutes";
            _xlWorkSheet.Cells[rowNum, 1].EntireRow.Font.Bold = true;
            ...
            _xlWorkSheet.Cells[rowNum, 3].NumberFormat = "#,##0.00";
            _xlWorkSheet.Cells[rowNum, 1].EntireRow.Font.Bold = true;

            Excel.Range _rangeA = (Excel.Range)_xlWorkSheet.Cells[starting_num, 1];
            Excel.Range _rangeB = (Excel.Range)_xlWorkSheet.Cells[rowNum, 3];
            _xlWorkSheet.get_Range(_rangeA, _rangeB).Borders.LineStyle = Excel.XlLineStyle.xlContinuous;

            _xlWorkSheet.get_Range("A1", "K100").Columns.AutoFit();
            _xlWorkSheet.get_Range("A1", "K100").Rows.AutoFit();
        ...
        {
            int rn = 0;
            _xlWorkSheet.Cells[2, 9] = "Diff (%)";
            if (rowNum1 == rowNum2)
            {
                for (int i = 3; i <= rowNum1; i++)
                {
                    if(greater == "LEFT")
                        _xlWorkSheet.Cells[i, 9] = "=(C" + i.ToString() + "-G" + i.ToString() + ")/C" + i.ToString() + "*100";
                    else if(greater == "RIGHT")
                        _xlWorkSheet.Cells[i, 9] = "=(G" + i.ToString() + "-C" + i.ToString() + ")/C" + i.ToString() + "*100";

                    _xlWorkSheet.Cells[i, 9].NumberFormat = "#,##0.0000";

                    double d = Convert.ToDouble(_xlWorkSheet.Cells[i, 9].Value2.ToString());
                    if (d > General_acceptance_diff || d < General_acceptance_diff * (-1))
                    {
                        _xlWorkSheet.Cells[i, 9].EntireRow.Font.Color = Excel.XlRgbColor.rgbRed;
                    }
                }
                _xlWorkSheet.Cells[rowNum1, 9].Font.Bold = true;
            }
            else
            {
                if (greater == "LEFT")
                    _xlWorkSheet.Cells[3, 10] = "Due to mismatch of row numbers, auto difference is not possible. Please do it manually. At first match the cells then use this formula: (C-G)/C*100.";
                else if (greater == "RIGHT")
                    _xlWorkSheet.Cells[3, 10] = "Due to mismatch of row numbers, auto difference is not possible. Please do it manually. At first match the cells then use this formula: (G-C)/C*100.";

                _xlWorkSheet.Cells[3, 10].Font.Color = Excel.XlRgbColor.rgbRed;
            }

            Excel.Range _rangeA = (Excel.Range)_xlWorkSheet.Cells[2, 9];
            Excel.Range _rangeB;

            if (rowNum1 > rowNum2)
            {
                _rangeB = (Excel.Range)_xlWorkSheet.Cells[rowNum1, 9];
                rn = rowNum1;
            }
            else
            {
                _rangeB = (Excel.Range)_xlWorkSheet.Cells[rowNum2, 9];
                rn = rowNum2;
            }
