...
namespace EReport
{
    class ViewModel : INotifyPropertyChanged
    {
        ...
        public bool EmailSendorNot = true;
        ...
        public void TaskHandle(String _generator, bool error_checkOrNot)
        {
            if (_generator == "AUTO" && error_checkOrNot)
            {
                ...
                IDDincoming_Diff_actual = IDDIncomingDifferenceCheck();
        ...
        private async Task TryToMail()
        {
            ...
            while (replymail == false)
            {
                ...
                count++;
                if (replymail == false && EmailSendorNot)
                {
                    if (count == 5)
                    {
        ...
        private bool MailReport()
        {
            ...
            try
            {
                ...
                _mailbody += "\n\n[It is autogenerated mail from Auto Report Generator application. Please do not reply with this mail.]";

                _mailbody += "\n\n{Powered by: Md. Rakib Subaid\nManager, Dhaka Multi Exchange\nIT & Billing, BTCL\nSher-E-Bangla Nagar, Dhaka}";
        ...
        {
            return Task.Run(() => DBW.QueryDatabaseforIDDIncoming(SubtractiveDataDay, _dir, General_acceptance_diff));
        }

        private Task <String> TaskHandleAsyncIDDOutgoing(string _dir)
        {
            return Task.Run(() => DBW.QueryDatabaseforIDDOutgoing(SubtractiveDataDay, _dir, General_acceptance_diff));
        }

        private Task<String> TaskHandleAsyncANSIncoming(string _dir)
        {
            return Task.Run(() => DBW.QueryDatabaseforANSLocalIncoming(SubtractiveDataDay, _dir, General_acceptance_diff));
        }

        private Task<String> TaskHandleAsyncANSOutgoing(string _dir)
        {
            return Task.Run(() => DBW.QueryDatabaseforANSLocalOutgoing(SubtractiveDataDay, _dir, General_acceptance_diff));
        }

        private Task<String> TaskHandleAsyncICX(string _dir)
        {
            return Task.Run(() => DBW.QueryDatabaseforICX(SubtractiveDataDay, _dir));
        }
        ...
        public Double IDDIncomingDifferenceCheck()
        {
            return DBW.IDDIncomingDifferenceCheck(SubtractiveDataDay);
        }
